...
router.post('/:reviewId/:reporterId', async (req, res) => {
  ...
  try {
    ...
    if (!isValidObjectId(reviewId) ||!isValidObjectId(reporterId)  ) {
      return res.status(400).json({ error: 'Invalid reporterId or reviewId format.' });
    }

    const newReport = await reportReviewService.reportReview(reviewId,reporterId, reason);
